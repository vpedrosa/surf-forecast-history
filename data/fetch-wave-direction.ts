...
async function fetchWaveDirectionForStation(stationId: number): Promise<WaveDataPoint[]> {
...
async function fetchAllWaveDirectionData() {
  try {
    ...
    console.log(`🧭 Parámetro: Dirección de procedencia (dmd)\n`);
    console.log('🌊 Iniciando descarga de datos históricos de dirección...\n');
    ...
    for (let i = 0; i < stations.length; i++) {
      ...
      try {
        ...
        console.log(`         ⏳ Descargando datos de dirección...`);

        const waveData = await fetchWaveDirectionForStation(station.id);
        const dataCount = waveData.length;
        totalDataPoints += dataCount;

        // Guardar datos en archivo
        const outputFile = join(__dirname, `${station.id}-direction.json`);
        writeFileSync(outputFile, JSON.stringify(waveData, null, 2), 'utf-8');

        console.log(`         ✅ Datos obtenidos: ${dataCount} puntos`);
        console.log(`         💾 Guardado en: ${station.id}-direction.json`);
